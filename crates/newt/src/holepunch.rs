...
/// sealed `{newtId, token, publicKey}`, wrapped in the envelope the exit node
/// expects. The exit node uses this to track the connector's live UDP endpoint;
/// the WireGuard tunnel alone is not enough for it to keep reporting the site.
pub fn build(newt_id: &str, token: &str, newt_pub_b64: &str, server_pub_b64: &str) -> Option<Vec<u8>> {

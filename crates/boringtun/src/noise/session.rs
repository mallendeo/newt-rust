...
impl Session {
    ...
    pub(super) fn format_packet_data<'a>(&self, src: &[u8], dst: &'a mut [u8]) -> &'a mut [u8] {
        ...
        let n = {
            ...
            self.sender
                ...
                .map(|tag| {
                    data[src.len()..src.len() + AEAD_SIZE].copy_from_slice(&tag);
                    src.len() + AEAD_SIZE
                })
                .unwrap()
        };
    ...
    pub(super) fn receive_packet_data<'a>(
        ...
        let pt_len = {
            ...
            // Ciphertext is followed by a 16-byte tag; decrypt the front in place.
            let split = ct_len - AEAD_SIZE;
            let tag = *chacha20poly1305::Tag::from_slice(&dst[split..ct_len]);
            self.receiver
